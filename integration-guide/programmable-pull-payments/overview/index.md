...
The SDK accepts `validationProgram = PublicKey.default()` and rewrites it to `SystemProgram.programId` internally (see `getCreateComposablePolicyInstruction`). When forward is disabled, `num_data_checks` must be `0` and `input_mint` must equal `output_mint` (no conversion step — it's a same-mint pull → sweep).
...
- [SDK surface](https://docs.tributary.so/integration-guide/programmable-pull-payments/sdk/index.md) — `getCreateComposablePolicyInstruction`, `executeComposable`, `ForwardConfig`, `ValidationConfig`.
