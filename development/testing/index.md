...
1. **`calculate_next_payment_due` is monotonic AND chrono-accurate** — result is strictly > now AND lands on the exact date chrono produces via iterative month-adding with per-step clamping.
1. **Iteration cap enforced** — `skip_months` bails at 1200 iterations with `ArithmeticOverflow`.
