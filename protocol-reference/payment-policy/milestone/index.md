...
1. **Waiting** — `current_milestone` timestamp and/or signer condition not yet met
1. **Execution** — when conditions are met, anyone calls `execute_payment`
1. **Transfer** — the current milestone amount moves to recipient, minus fees
1. **Advance** — `current_milestone` increments
1. **Completion** — after the last milestone, the policy auto-pauses
...
1. If bit 1 is set (`RELEASE_GATEWAY`): verify gateway authority signed the transaction
1. If bit 2 is set (`RELEASE_OWNER`): verify the policy owner (payer) signed
1. If bit 3 is set (`RELEASE_RECIPIENT`): verify the recipient signed
