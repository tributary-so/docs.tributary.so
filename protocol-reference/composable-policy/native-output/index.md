...
Fees are still taken in **WSOL** via `transfer_checked` into the gateway and protocol fee accounts *before* the `closeAccount` fires. The `sweep_amount` returned for accounting is the WSOL value unwrapped; the rent bonus shipped by `closeAccount` is excluded from `composable_policy.total_output`.
