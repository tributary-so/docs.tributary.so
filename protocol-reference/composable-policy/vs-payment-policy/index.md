...
A `ComposablePolicy` with `target_program = Pubkey::default()` and `validation_program = SystemProgram` is functionally close to a `PaymentPolicy` — same pull, same fee split, same schedule math — but:
